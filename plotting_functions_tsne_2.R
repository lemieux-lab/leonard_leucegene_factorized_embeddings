...
FE_tsne = read.csv(paste(wd, paste(mid, "_train_test_tsne.txt", sep = ""), sep = "/"))
...
p = ggplot(FE_tsne, aes(x = tsne_1, y = tsne_2, col = interest_group, shape = train_test)) + geom_point() + coord_fixed() + 
  theme_classic() + scale_color_manual(values = c("red", "grey", "blue"))
g = ggplot(FE_tsne, aes(x = tsne_1, y = tsne_2, col = cyto_group), shape = train_test) + geom_point() + coord_fixed() + 
  theme_classic() 
