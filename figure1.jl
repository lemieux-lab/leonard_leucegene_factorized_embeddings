...
inf_pos, positions = inference(X_t, Y_t, model, tr_params, dump_cb; nepochs_tst = 100, ns = 50)
inf_pos
true_pos = model.embed_1.weight[:,selected_sample]
...
run(`Rscript --vanilla plotting_predicted_pos.R $outdir $(params.modelid) $(cf_df.sampleID[selected_sample]) $(true_pos[1]) $(true_pos[2])`)
